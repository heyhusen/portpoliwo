<template>
	<header class="py-2">
		<h1 class="font-bold text-2xl sm:text-3xl">Create Portfolio Category</h1>
	</header>
	...
	<section class="flex flex-col sm:flex-row gap-6 md:gap-8">
		<header class="space-y-1 sm:max-w-2/5">
			...
			<small class="text-sm text-gray-500">
				Add a category to your portfolio, to make it more organized.
			</small>
		</header>
...
const onSubmit = handleSubmit((values) => {
	loading.value = true;
	api
		.post('/portfolio/category', values)
		.then(({ data: success }) => {
